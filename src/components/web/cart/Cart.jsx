...
export default function Cart() {
    ...
    const {getCartContext,removeItemContext,clearCartContext,incraseQuantity,decraseQuantity}=useContext(cartContext)
...
const increase=async(productId)=>{
  const res=await incraseQuantity(productId);
  return res;
}

const decrease=async(productId)=>{
  const res=await decraseQuantity(productId);
  return res;
}

  return (
    <div className="cart lobster bg1">
          ...
          <div className="products" id="products">
           ...
           {data?.products?(
            ...
            data.products.map((product)=>
                ...
                <div className="subtotal">${product.details.price}{product.details.count}</div>
              </div>  
            )
        ):<h2>no Product</h2>}

        <button className='btn btn-info w-50 m-auto my-5 border' onClick={()=>removeAll()}>Remove all</button>
